...
function install_node() {
  ...
  # 让用户输入想要创建的容器数量
  read -p "请输入你想要创建的节点数量，单IP限制最多5个节点，默认1个: " container_count

  # 让用户输入起始 RPC 端口号
  read -p "请输入你想要设置的起始 RPC端口 （端口号请自行设定，开启5个节点端口将会依次数字顺延，默认 30000）: " start_rpc_port

  # 让用户输入想要分配的空间大小
  read -p "请输入你想要分配每个节点的存储空间大小（GB），一般 50 即可, 网页生效较慢，等待20分钟后，网页查询即可: " storage_gb
